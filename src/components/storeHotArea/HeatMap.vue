  ...
  methods: {
    initData() {
      ...
      if (!_.isEmpty(this.heatData)) {
        ...
        this.storeHeatmap.heatmapPoints = this.getHeatMapPoints()
    ...
    getIntervalList() {
      // 获取所有的value并且从小到大排序
      let pointValueList = _.map(
        this.storeHeatmap.heatmapPoints,
        o => o.value
      ).sort(function(a, b) {
        return a - b
      })
    ...
    setHeatMapInstance() {
      ...
      let heatmapPointData = {
        // 设置热点集合的最大值
        // 'max': this.storeHeatmap.max,
        max: this.maxValue / this.dayDiff,
        min: 1,
        data: this.storeHeatmap.heatmapPoints
      }
    ...
    // 热区的颜色配置
    getGradient() {
      this.storeHeatmap.max = this.getHeatMapMax()
